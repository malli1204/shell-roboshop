...
do
    ...
    if [ $instance != "frontend" ]
    then
        IP=$(aws ec2 describe-instances --instance-ids i-0e56c981d83d8bc75 --query "Reservations[0].Instances[0].PrivateIpAddress" --output text)

    else
        IP=$(aws ec2 describe-instances --instance-ids i-0e56c981d83d8bc75 --query "Reservations[0].Instances[0].PublicIpAddress" --output text)
